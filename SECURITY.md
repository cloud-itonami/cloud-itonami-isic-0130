...
If you discover a security issue in cloud-itonami-isic-0130, **do not open a
...
This repository implements the **Governor** for plant-propagation-nursery
...
1. **Hard violations in Governor rules must never be overridable** — a rule
   marked as HARD (e.g., `:already-logged`, `:rooting-rate-below-minimum`,
   `:pest-or-disease-detected`) blocks the proposal unconditionally
2. **Escalation gates must trigger without race conditions** — quarantine
...
- Include documentation of the propagule-viability/quality rationale
